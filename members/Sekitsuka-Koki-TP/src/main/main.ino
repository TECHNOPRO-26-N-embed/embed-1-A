...
void updateInputBuffer(char key) {
  if (inputLength >= 4) {
    playWarningTone();
    blinkLed(PIN_LED_RED, 100); // 入力制限通知
    return;
  }
  ...
  Serial.println(); // デバッグ用に現在の入力バッファを日本語で表示
  blinkLed(PIN_LED_GREEN, 100); // 入力フィードバック
}
...
void applyEditKey() {
  if (inputLength > 0) {
    ...
    Serial.println(" (削除)"); // デバッグ用に削除後の入力バッファを日本語で表示
    blinkLed(PIN_LED_YELLOW, 100); // 削除フィードバック
  }
}
...
void startJudgeIfReady() {
  if (inputLength == 4) {
    attemptCount++;
    judgeAndStoreResult();
    currentState = 1; // 判定表示状態へ
  } else {
    playWarningTone();
    currentState = 3; // ミス入力通知状態へ
  }
}
...
void judgeAndStoreResult() {
  ...
  // 判定結果をO/A/Xで表示（O=一致, A=位置違い, X=不一致）
  Serial.print("[" );
  Serial.print(attemptCount);
...
void playClearWave() {
  ...
  resetGame(); // クリア後は新しい正解コードで次ゲームを開始
}
