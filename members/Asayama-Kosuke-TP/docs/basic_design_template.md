...
| — | 初期化 | `setup()` | ピン設定 |  |  |  |
| — | メイン制御 | `loop()` | 状態に応じて処理 |  |  |  |
| — | 入力読取 | `readKeypad()` | キー入力取得 |  |  |  |
| — | 判定処理 | `checkPassword()` | パスワード一致確認 |  |  |  |
| — | 出力制御 | `updateOutput()` | LED・ブザー制御 |  |  |  |
| F01 | 数字入力 | `inputNumber()` | 入力配列に追加 |  |  |  |
| F02 | 正解処理 | `successAction()` | LED点灯 |  |  | |
| F03 | エラー処理 | `errorAction()` | ブザー鳴動 |  |  |  |
| A01 | 入力確定 | `confirmInput()` | #で判定 |  |  |  |
| A02 | ロック機能 | `lockSystem()` | 一定時間ロック |  |  |  |
| A03 | エラー通知 | `beepError()` | 音鳴らす |  |  |  |
