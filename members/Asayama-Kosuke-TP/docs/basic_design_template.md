# 基本設計書 — 組込み開発実習

<!-- 作成者: あなたの名前 / 日付: YYYY-MM-DD / グループ: 〇-〇 -->

> **このドキュメントの目的**
> 要件定義書（requirements.md）で「**何を作るか**」を決めました。
> この基本設計書では「**どのような構造で作るか**」を決めます。
> コードは書きません。図・表・テキストで「設計の骨格」を作るドキュメントです。

> [!NOTE]
> **V字モデルにおける位置づけ**
> 基本設計書 ←→ **結合テスト**（部品を組み合わせて必須機能が動くか）が対応します。
> Section 6 の結合テスト計画は、設計を終えた後に記入します。

---

## 0. 要件定義書との接続確認

> ※ まず要件定義書（requirements.md）の内容を確認・転記します。

| 項目 | requirements.md から転記 |
|:--|:--|
| 作品タイトル |暗証番号ロック装置 |
| 一言説明（何ができるガジェットか） | 暗証番号入力でロックを解除する装置|
| 使用する共通部品（グループ共通） | membrane switch module|
| 追加部品（個人・主なもの） |Arduino UNO, LED, ブザー, 抵抗 |
| 必須機能の数（3-1の件数） | 　3件 |
| 追加機能の数（3-2の件数） | 　3件（未実装でも設計しておく） |

> [!IMPORTANT]
> 要件定義書の「やらないこと（3-3）」に書いたことは、この設計書に登場させないこと。
> 設計中にアイデアが増えた場合は、まず要件定義書の「任意機能（3-2）」に追加して確認する。
> **追加機能も設計書に含めること。** 優先度が低いだけで、構造は先に考えておく。

---

## 0-2. グループ共通部品との接続設計

> ※ グループで決めた「共通部品」と、あなたの個人設計の接点を明確にします。

| 項目 | 内容 |
|:--|:--|
| グループ共通部品（名称） |membrane switch module |
| 共通部品が担う役割 | 数字入力（暗証番号入力） |
| 接続するArduinoピン（予定） | D2〜D9（キーパッド用） |
| 個人の設計と共通部品が協調する箇所 | 入力された数字を配列に保存し、4桁入力後に判定する |
| グループ内で統一すべき仕様 | ピン配置、キー配列（数字配置）、確定キーは「#」 |

> [!TIP]
> グループメンバー全員が同じピン番号・同じ閾値の単位を使っているか、
> グループレビューで確認し合ってください。

---

## 1. システム全体像

### 1-1. ブロック構成（入力 → 処理 → 出力）

> ※ Arduinoを中心に「何が入力で何が出力か」をテキストの図で整理します。

**記入例：**
```
【入力】                 【Arduino UNO R3】            【出力】
タクトスイッチ ─────→┐                         ┌──→ LED（赤・緑）
超音波センサー ─────→┤   状態管理・判定処理    ├──→ パッシブブザー
                      └─────────────────────────┘
```

**↓ 自分のシステムを書いてください**
```
【入力】                 【Arduino UNO R3】            【出力】
キーパッド ─────→┐                        ┌──→ LED
                  ├── 入力処理・判定処理 ──┤
                  └──────────────────────┘
                                     └──→ ブザー

```

補足説明（図で表現しきれない場合）：
キーパッドからの入力を受け取り、正誤判定を行い、LEDとブザーで結果を通知する。
---

### 1-2. 状態遷移（ガジェットの「ふるまい」設計）

> ※ ガジェットが「どんな状態を持つか」を定義します。
> この設計が loop() の骨格になります。

**記入例：**
```
[電源ON / 初期化]
  ↓（初期化完了）
[待機中] ──（センサー検知）──→ [計測中] ──（閾値超え）──→ [警告中]
   ↑                                                            │
   └────────────────────（リセットボタン）────────────────────────┘
```

**↓ 自分の状態遷移を書いてください**
```
[電源ON / 初期化]
  ↓（初期化完了）
[待機中] ──（数字入力）──→ [入力中] ──（#押下）──→ [判定]
 ↑                                                ↓
  └────────（リセット）─────────── [エラー / 正解]

```

**各状態の意味：**

| 状態名 | この状態のとき、何をしているか | LEDやブザーなど出力の様子 |
|:--|:--|:--|
| 待機中 | 入力待ち状態 | 消灯 |
| 入力中 | 数字を入力している | 特に変化なし |
| 判定 | 入力された4桁をチェック | 一瞬処理 |
| 正解 | 正しい暗証番号 | LED点灯 |
| エラー | 間違い | ブザー鳴動 |

> [!TIP]
> すべての状態から「抜け出せるルート」があるか確認してください。
> 「このボタンを押せば必ずリセットに戻る」が保証されていますか？

---

## 2. ソフトウェア設計

### 2-1. データ設計（使う変数・定数の洗い出し）

> ※ プログラムで扱う「情報の種類」を先に整理します。
> ソフトウェアワークシート（AI駆動開発）でやった「データの洗い出し」と同じです。

| 情報の名前 | 何を表すか | 型（イメージ） | バイト数 | 初期値 | 備考 |
|:--|:--|:--|:--|:--|:--|
| currentState | 現在の状態 | int | 2B | 0 | |
| inputCode | 入力された暗証番号 | char[5] | 5B | "" | 4桁+終端 |
| correctCode | 正解コード |char[5] | 5B | "1234" |仮設定 |
| inputIndex | 入力位置 | int | 2B | 0 | |
| lastMillis | タイマー | unsigned long | 4B | 0 |  |
| errorCount | ミス回数 | int | 2B | 0 |  |

> [!CAUTION]
> **SRAM使用量チェック（Arduino UNO R3 の上限は 2048B）**
>
> グローバル変数の合計: 20B
>
> | 合計バイト数 | 判定 |
> |:--|:--|
> | ～1000B | ✅ 余裕あり |
> | 1000～1500B | ⚠️ char配列の長さを見直す |
> | 1500B超 | 🔴 AIに相談し変数を整理すること |

### 2-2. 機能一覧・関数一覧（役割の定義）

> ※ プログラムを「処理の部品（関数）」に分割します。
> **ここではコードを書かず、「何をする関数か」だけ日本語で書いてください。**
> 具体的な実装は詳細設計書（detailed_design.md）で行います。

> [!TIP]
> **単一責任の原則（1関数 = 1つの仕事）**
> 「機能名」欄に requirements.md の機能名（必須・追加）を書き、それを実現する関数名を対応させてください。
> 入出力の読み取り（readButton 等）や制御（updateOutput 等）は複数機能から使われる共通処理として「共通」と書いてOKです。

| 機能ID | 機能名 | 関数名 | 担う「1つの仕事」 | 主な引数 | 戻り値 | 呼び出す場所 |
|:--|:--|:--|:--|:--|:--|:--|
| — | 初期化 | `setup()` | ピン設定 | なし | なし | 起動時1回 |
| — | メイン制御 | `loop()` | 状態に応じて処理 | なし | なし | 常時 |
| — | 入力読取 | `readKeypad()` | キー入力取得 | なし | char | loop()内 |
| — | 判定処理 | `checkPassword()` | パスワード一致確認 | char[] | bool | confirmInput()内 |
| — | 出力制御 | `updateOutput()` | LED・ブザー制御 | int state | なし | loop()内 |
| F01 | 数字入力 | `inputNumber()` | 入力配列に追加 | char key | なし | loop()内 |
| F02 | 正解処理 | `successAction()` | LED点灯 | なし | なし | checkPassword()後 |
| F03 | エラー処理 | `errorAction()` | ブザー鳴動 | なし | なし | checkPassword()後 |
| A01 | 入力確定 | `confirmInput()` | #で判定 | なし | なし | loop()内 |
| A02 | ロック機能 | `lockSystem()` | 一定時間ロック | なし | bool（ロック中か） | loop()内 |
| A03 | エラー通知 | `beepError()` | 音鳴らす | int 回数 | なし | errorAction()内 |

> [!CAUTION]
> loop() の中で全部書こうとしていませんか？
> 機能ごとに関数を分けることで、グループレビューや単体テストがしやすくなります。

### 2-3. タイミング設計（delay vs millis）

> ※ `delay(N)` はN ms間、**すべての処理**が止まります。
> 複数の処理を並行させたい場合は `millis()` を使います。

| 処理 | 必要な周期 | delay / millis | 判断の理由 |
|:--|:--|:--|:--|
| キー入力監視 | 常時 | **millis** | 常に反応する必要 |
| LED制御 | 即時 | **millis** | 非停止制御 |
| ブザー | 200ms |  **millis** | 他処理と並行 |

> [!CAUTION]
> `delay()` を使う場合、その間はボタン入力もセンサー読み取りも**すべて止まります**。

---

## 3. ハードウェア設計

### 3-1. 部品一覧とピン接続

> ※ デジタルピン: D2〜D13 / アナログピン: A0〜A5
> 特殊用途: D0/D1（シリアル通信・使用禁止）、A4/A5（I2C）、D10〜D13（SPI）

| 部品名 | 型番・種類 | Arduinoピン | 入力 / 出力 | 備考 |
|:--|:--|:--|:--|:--|
| キーパッド | membrane | D2〜D9 | 入力 |  |
| LED | 5mm | D10 | 出力 | 220Ω |
| ブザー | passive | D11 | 出力 |  |


> [!CAUTION]
> **ピン競合チェック（必須）**
> - D0/D1 は Serial 通信専用のため使用禁止
> - I2C 機器（LCD等）は A4/A5 を使用
> - 同じピンに複数の部品を繋いでいないか確認

**ピン競合チェック結果：** 競合なし / 競合あり → （内容）

### 3-2. 電気的考慮事項

> **【任意】** 余裕があれば記入してください。抵抗値はAIに計算させてOKです。

| 用途 | 判断・計算メモ |
|:--|:--|
| LED抵抗 | (5V-2V)/0.02A=150Ω → 220Ω |
| キーパッド |内部プルアップ使用 |
| ブザー | 直接接続 |

---

## 4. 異常系・エラー処理の方針

| 異常ケース | 発生条件 | 検知方法 | 対応方針 |
|:--|:--|:--|:--|
| チャタリング | 高速入力 | 時間差 | 無視 |
| 入力ミス | 不一致 | 比較 | エラー音 |
| 過剰ミス | 3回以上 | カウント | 一時ロック |
| 未入力 | 空 | 長時間 | リセット |

---

## 5. 要件 ←→ 設計対応表（トレーサビリティ）

> **【任意・推奨】** Section 1〜4 が完成してから記入します。
> 「設計箇所」が空欄の行は、設計が漏れているサインです。

| No | 必須機能（requirements.md 3-1 から転記） | 対応するSW設計（関数名等） | 対応するHW設計（ピン等） | 結合テスト No |
|:---|:---|:---|:---|:---|
| 1 | 数字入力 | 2-2（関数：　inputNumber　） | 3-1（ピン：keypad） | テスト#1 |
| 2 | 正解でLED点灯 | 2-2（関数：　successAction　） | 3-1（ピン：D10） | テスト#2 |
| 3 | エラー通知 | 2-2（関数：　errorAction　） | 3-1（ピン：D11） | テスト#3 |

---

## 6. 結合テスト計画（V字モデル：基本設計 ↔ 結合テスト）

> ※ 実装完了後に「必須機能が動いているか」を確認するためのテスト計画です。
> 「実際の結果」欄は実装後に記入します。

| No | テスト対象（requirements.md の必須機能から転記） | テスト手順（操作方法） | 期待する結果 | 実際の結果 | 合否 |
|:---|:---|:---|:---|:---|:---|
| 1 | 数字入力 | 1234入力 | 記録される | ← 実装後に記入 | [ ] |
| 2 | 正解 | 正しい番号 | LED点灯 | | [ ] |
| 3 | エラー | 間違い | ブザー鳴る | | [ ] |
| 4 | チャタリング | 連打 | 1回だけ反応する |  | [ ] |
| 5 | 異常入力 | 何もしない | 安定 |  | [ ] |

---

## 7. AIレビュー記録

> グループレビューの前に必ず実施してください。

### Q1: 設計の妥当性確認

> 「この基本設計書をもとに Arduino UNO R3 でコードを書きます。ピン競合・ライブラリ競合・状態遷移の抜け・delay() の問題点はありますか？」

**AIの回答（要約）：**

ピン競合なし
状態遷移もシンプルで問題なし
delay未使用で設計良好

**対応した内容：**

---
### Q2: チャタリング・delay・ノイズの確認

> 「この設計にチャタリング対策は含まれていますか？delay() を使っている箇所で問題はありますか？アナログセンサーを使う場合、ノイズ対策は必要ですか？」

**AIの回答（要約）：**

チャタリング対策あり
delay未使用でOK
ノイズ問題なし（デジタル入力のため）

**対応した内容：**

---

### Q3: 要件への対応漏れ確認（任意）

> **【任意】** トレーサビリティ表（Section 5）を書いた場合に実施してください。
>
> 「要件定義書の必須機能と、この基本設計書のSection 5 を比較して、設計漏れや結合テスト計画の抜けを教えてください。」

**AIの回答（要約）：**

**対応した内容：**

---

## 8. グループレビュー記録

### 8-1. 指摘一覧

| No | 指摘内容 | 指摘者 | 対応 |
|:---|:---|:---|:---|
| 1 | 機能一覧の引数、戻り値などの記入漏れ | 藤原 | 追記した |
| 2 | ブザー以外に追加実装を足さないの？ | 関塚 | 時間があればパスワードを自分で設定できる機能を追加したい |
| 3 |  |  |  |

### 8-2. レビューを受けて変更した点

-
-

---

*初版: YYYY-MM-DD / AIレビュー: YYYY-MM-DD / グループレビュー後更新: YYYY-MM-DD*
